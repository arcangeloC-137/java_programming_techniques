	...
	public List<Airport> trovaPercorso(Airport a1, Airport a2){
		...
		it.addTraversalListener(new TraversalListener<Airport, DefaultWeightedEdge>(){
			...
			public void edgeTraversed(EdgeTraversalEvent<DefaultWeightedEdge> e) {
				/*
				 * L'unico metodo utile è questo, dove salviamo la relazione
				 * padre/figlio tra nodo sorgente e destinazione nell'albero di visite, 
				 * che per noi è una mappa
				 */
				...
				if(!visita.containsKey(destinazione) && visita.containsKey(sorgente)) {
					/*
					 * Se questo è vero, nel nostro albero di visita abbiamo il nodo 
		...
		while(!step.equals(a1)) {
			//risalgo l'albero di visita finché non ritrovo il nodo di partenza
			percorso.add(step);
